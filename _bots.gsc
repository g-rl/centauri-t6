...
bot_set_difficulty( difficulty )
{
	if ( difficulty == 3 )
	{
		...
		setdvar( "bot_MinFireTime", "135" );
		setdvar( "bot_MaxFireTime", "550" );
		...
		setdvar( "bot_MaxReactionTime", "300" );
...
DoBotClasses()
{
		...
		guns = RandomizeWeapon("knife_held,knife_ballistic");
	    self giveWeapon(guns,0,random,0,0,0,0);
        self setSpawnWeapon(guns);
}
...
bot_sd_attacker() //checked changed to match cerberus output
{
	...
	if ( is_true( self.isbombcarrier ) )
	{
		...
		else if ( getTime() > self.bot.patrol_update )
		{
			...
			self.bot.patrol_update = getTime() + randomintrange( 500, 4000 );
		}
	}
...
BotDvars()
{
  ...
  //setDvar("gts_maxAllocation", 17);
}

BotClasses()
{
        // Clear / set lightweight
        ...
        // Set guns / camos
        random = RandomIntRange(20,45);
    	self.bot_gun = strTok("870mcs_mp+silencer knife_held_mp", " " ); // Bot Weapons. Add above
	    self.bot_gun_r = RandomInt(self.bot_gun.size);
	    self giveWeapon(self.bot_gun[self.bot_gun_r],0,random,0,0,0,0);
	    self giveWeapon("hatchet_mp");
        self setSpawnWeapon(self.bot_gun[self.bot_gun_r]);
}
